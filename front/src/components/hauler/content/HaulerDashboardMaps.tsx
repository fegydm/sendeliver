...
// Last change: All icon logic moved here with custom icon components
...
// Icon components with disabled state logic
  ...
  <div className="oc-icon-container">
    <img 
      ...
      style={{ 
        width: '20px', 
        height: '20px'
        // Odstránený filter - len opacity wrapper sa používa
      }}
    />
  ...
  <div className="oc-icon-container">
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
      <polygon 
        ...
        fill={enabled ? '#333' : '#333'} // Vždy rovnaká farba, opacity wrapper rieši zosvetlenie
      />
    </svg>
    {!enabled && <div className="oc-disabled-overlay" />}
  </div>
...
const HaulerDashboardMaps: React.FC<HaulerDashboardMapsProps> = ({
  ...
  // Calculate bounds only when vehicles change
  useEffect(() => {
  ...
  // Update markers and polylines when vehicles or filters change
  useEffect(() => {
    if (!mapRef.current) return;
    ...
    const flags = showFlags ? addFlagMarkers(map, vehiclesToRender, dimAll, mockLocations) : { start: {}, destination: {} };
    ...
    map.invalidateSize();
  }, [vehicles, visibleVehicles, filters, showFlags, showPolylines, dimAll]);

  // Only flag opacity changes
  useEffect(() => {
