...
// Last change: Added openSlider state for exclusive slider expansion
...
const HaulerDashboardMaps: React.FC<HaulerDashboardMapsProps> = ({
  ...
  useEffect(() => {
    ...
    const map = L.map(mapDiv.current, { zoomAnimation: false, fadeAnimation: false }).setView([49, 15], 6);
    mapRef.current = map;
  ...
  useEffect(() => {
    ...
    map.invalidateSize();
  }, [vehicles, visibleVehicles, filters, showFlags, flagOpacity, showPolylines, polylineOpacity, isChartExpanded, isVehiclesExpanded, hover]);
    ...
    <div className="dashboard__map-container">
      ...
      <div className="map-controls-container">
        <OpacityControl
          ...
          toggleIcon={<img src={FinishFlag} alt="Finish Flag" className="h-5 w-5" />}
          arrowIcon={<span>▼</span>}
          className="map-flag-control"
        ...
        <OpacityControl
          ...
          arrowIcon={<span>▼</span>}
          className="map-polyline-control"
          openSlider={openSlider}
          setOpenSlider={setOpenSlider}
        />
      </div>
