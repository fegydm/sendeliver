...
const MessageBanner: React.FC<MessageBannerProps> = () => {
  ...
  useEffect(() => {
    ...
    const setupBanner = () => {
      ...
      const currentVerification = JSON.parse(localStorage.getItem(PENDING_VERIFICATION_KEY) || '{}');

      console.debug('[MessageBanner] Current verification state:', { currentVerification, localStorage: localStorage.getItem(PENDING_VERIFICATION_KEY) });

      // Zobraz banner iba ak existuje platná verifikácia v localStorage
      if (currentVerification?.email && currentVerification.expiresAt > new Date()) {
        ...
        console.debug('[MessageBanner] Banner shown from localStorage:', { message: state.bannerMessage, timeLeft, totalTime: state.totalTime });
  ...
  const handleCloseBanner = useCallback(() => {
    dispatch({ type: 'SET_SHOW_BANNER', payload: false });
    console.debug('[MessageBanner] Banner closed manually, localStorage preserved');
  }, []);

  const handleResendEmail = useCallback(async () => {
    const currentVerification = JSON.parse(localStorage.getItem(PENDING_VERIFICATION_KEY) || '{}');
    if (!currentVerification?.email) {
      console.debug('[MessageBanner] Resend email skipped: No email in localStorage');
      return;
    }
    ...
    try {
      ...
      dispatch({ type: 'SET_TOTAL_TIME', payload: 15 * 60 });
      console.debug('[MessageBanner] Verification email resent, updated localStorage:', { newVerification });
    } catch (err: any) {
  ...
  const handleCodeKeyPress = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && state.verificationCode.length === 6 && isValidCode(state.verificationCode)) {
