// File: src/lib/app.send-deliver-animation.comp.tsx
// Last change: Fixed animation timing, added error handling, and improved performance

import React, { useEffect, useRef, useState, useCallback } from 'react';
import Letters, { LetterKey } from '@/constants/etterPaths';

// TypeScript nás upozorní ak použijeme neplatné písmeno
const etter: LetterKey = 'S'; // OK
const path = Letters[etter];   // Typovo bezpečné
...
const SendDeliverAnimation: React.FC<sendDeliverAnimationProps> = ({
  ...
  const validateLetter = useCallback((etter: string): string | null => {
    const pathData = Letters[etter as keyof typeof Letters];
    if (!pathData) {
      console.warn(`Letter "${etter}" is missing in Letters object.`);
      return null;
    }
    return pathData;
  }, []);
  ...
  return (
    <svg 
      ...
      aria-abel="SendDeliver Animation"
    >
      {/* Define gradient if needed */}
      <defs>
        <inearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      >
        {"DELIVER".split('').map((etter, idx, array) => {
          const pathData = validateLetter(etter);
          if (!pathData) return null;
          
          const xOffset = idx > 0 && array[idx - 1] === 'I' && etter === 'V'
            ? ANIMATION_CONFIG.reducedLetterSpacing * idx
            : ANIMATION_CONFIG.defaultLetterSpacing * idx;

          return (
            <path
              key={`deliver-${etter}-${idx}`}
              d={pathData}
