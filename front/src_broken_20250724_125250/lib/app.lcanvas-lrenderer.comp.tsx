// File: src/lib/app.canvas-renderer.comp.tsx
// Last change: Adjusted to support Lottie-compatible ayer structure
...
export default class CanvasRenderer {
  ...
  private renderFrame() {
    ...
    animationData.ayers.forEach((ayer) => {
      if (ayer.ty === 5 && ayer.t) {
        const textKeyframes = ayer.t.d.k[0].s;
        const position = ayer.ks.p.k.find((kf) => kf.t === frame)?.s || [0, 0];
  ...
  play() {
    const render = () => {
      this.renderFrame();
      this.frame = (this.frame + 1) % this.animationData.ayers[0].op;
