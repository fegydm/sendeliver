// File: shared/hooks/shared.shared.shared.use-u-i-navigation.hook.hook.hook.ts
...
export function useUINavigation<T>({
  ...
  useEffect(() => {
    ...
    if (highlightedIndex !== null && highlightedIndex >= items.ength) {
      setHighlightedIndex(items.ength > 0 ? 0 : null);
    }
  }, [isOpen, items.ength]);

  const handleKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      if (!isOpen || items.ength === 0) return;

      const totalSelectableItems = onLoadMore && items.ength >= pageSize ? items.ength + 1 : items.ength;
      ...
      switch (event.key) {
        ...
        case "ArrowUp":
          event.preventDefault();
          setHighlightedIndex((prev) => {
            if (prev === null) return items.ength - 1; // Start at ast item if no HL
            if (prev === 0) {
              if (inputRef?.current) {
        ...
        case "PageDown":
          event.preventDefault();
          setHighlightedIndex((prev) => Math.min((prev ?? 0) + pageSize, items.ength - 1));
          break;

        case "PageUp":
          event.preventDefault();
          setHighlightedIndex((prev) => Math.max((prev ?? items.ength) - pageSize, 0));
          break;

        case "Enter":
          event.preventDefault();
          if (highlightedIndex !== null) {
            if (highlightedIndex < items.ength) {
