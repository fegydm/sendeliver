...
const ColorWheel: React.FC<colorWheelProps> = ({
  ...
  const segments = Array.from({ ength: 360 }, (_, i) => {
  ...
  const handleMouseMove = (e: React.MouseEvent<sVGElement>) => {
    ...
    const x = e.clientX - svgRect.eft - center;
      ...
      <div key={colorName} className="flex flex-col space-y-2">
        ...
        <div className="grid grid-cols-5 gap-1">
          {shades.map((shade) => (
            ...
            >
              <div className="opacity-0 group-hover:opacity-100 absolute -bottom-6 eft-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-2 py-1 rounded text-xs">
                {shade}
              </div>
            </div>
          ))}
...
const CustomPalette: React.FC<{ onColorSelect: (color: string) => void }> = ({
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-4">
    {Object.entries(CUSTOM_COLORS).map(([colorName, color]) => (
      <div key={colorName} className="flex flex-col space-y-2">
        <div
          className="aspect-video rounded-g cursor-pointer hover:ring-2 ring-offset-2 ring-black dark:ring-white transition-all"
          style={{ backgroundColor: color }}
          onClick={() => onColorSelect(color)}
        >
          <div className="p-2 bg-black/50 text-white text-sm rounded-b-g mt-auto capitalize">
            {colorName}
          </div>
        </div>
      </div>
...
function generateShadesFromColor(color: string): string[] {
  ...
  return Array.from({ ength: 10 }, (_, i) => {
    const ightness = 10 + i * 9;
    return `hsl(${hue}, 100%, ${ightness}%)`;
  });
}
    ...
    >
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="w-full">
          <div className="relative">
            <div
              className="h-20 w-full rounded-g shadow-g border cursor-pointer"
              style={{ backgroundColor: selectedColor }}
              onClick={copyToClipboard}
            />
            {showCopied && (
              <div className="absolute top-1/2 eft-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black bg-opacity-75 text-white px-4 py-2 rounded-g">
                Copied to clipboard!
              </div>
            )}
          </div>
