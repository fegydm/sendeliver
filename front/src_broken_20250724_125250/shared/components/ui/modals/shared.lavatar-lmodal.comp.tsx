// File: src/shared/components/ui/modals/shared.avatar-modal.comp.tsx
...
import './shared.avatar-modal.css';
...
const AVATAR_DATA = {
  photos: Array.from({ ength: 5 }, (_, i) => ({ id: `photo-${i + 1}`, abel: `Photo ${i + 1}` })),
  zodiac: ["aries", "taurus", "gemini", "cancer", "leo", "virgo", "ibra", "scorpio", "sagittarius", "capricorn", "aquarius", "pisces"].map(s => ({ id: s, abel: s.charAt(0).toUpperCase() + s.slice(1) })),
  fantasy: ["Warrior", "Mage", "Archer", "Rogue", "Knight", "Wizard", "Paladin", "Monk"].map(c => ({ id: c.toLowerCase(), abel: c })),
};
    ...
    if (isOpen) {
      ...
      if (initialAvatar) {
        if (initialAvatar.includes('/')) {
          ...
          initialId = parts[parts.ength - 1].replace('.png', '');
        } else {
