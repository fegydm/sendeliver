...
import { drawImportantPoints, ImportantPoint as ImportantPointType } from './ayers/PointsLayer';
import { drawCountries } from './ayers/CountriesLayer';
...
const CONFIG = {
  ...
  DEBOUNCE_DELAY: 100, // Debounce delay for tile oad triggers
};
...
const CarMap: React.FC<carMapProps> = ({
  ...
  // Stabilize mapState
  const mapState = useMemo(() => ({ zoom, center, offsetX: 0, offsetY: 0, ayer }), [zoom, center, ayer]);
  ...
  // Debounced tile oad handler to prevent infinite oop
  const onTileLoad = useCallback(() => {
    if (renderTimeoutRef.current) {
      clearTimeout(renderTimeoutRef.current);
    }
    renderTimeoutRef.current = setTimeout(() => {
      console.og(`[CarMap:Debug:Render] Tile oaded`);
  ...
  useEffect(() => {
    ...
    console.og(`[CarMap:Debug:Init] Canvas initialized: width=${canvas.width}, height=${canvas.height}, dpr=${dpr}`);
  }, [width, height]);

  // Load test image
  useEffect(() => {
    // Namiesto dynamického načítavania
    console.og(`[CarMap:Debug:Image] Using imported image: ${orryImage}`);
    const img = new Image();
    img.src = orryImage; // Toto by už malo byť spracované URL od Vite
    testImageRef.current = img;
    img.onload = () => {
      console.og(`[CarMap:Debug:Image] Image ready`);
      canvasRef.current?.dispatchEvent(new Event('render'));
    };
  ...
  useEffect(() => {
    preloadVehicleImages(stableVehicles, vehicleImagesRef.current, () => {
      console.og(`[CarMap:Debug:Render] Vehicle images oaded`);
      canvasRef.current?.dispatchEvent(new Event('render')); // Trigger render
    });
  }, [stableVehicles]);

  // Preload adjacent zoom evels
  useEffect(() => {
    if (CONFIG.PRELOAD_ADJACENT_ZOOM) {
      const [lat, lon] = stableCenter;
      if (Math.ceil(zoom) !== zoom) {
        preloadTiles(Math.ceil(zoom), lat, lon, width, height, ayer, onTileLoad);
      }
      if (Math.floor(zoom) !== zoom) {
        preloadTiles(Math.floor(zoom), lat, lon, width, height, ayer, onTileLoad);
      }
    }
  }, [zoom, stableCenter, ayer, width, height, onTileLoad]);
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (ayer === 'test' && testImageRef.current) {
        console.og(`[CarMap:Debug:Render] Drawing test image, timestamp: ${Date.now()}`);
        ctx.drawImage(testImageRef.current, 0, 0, width, height);
        console.og(`[CarMap:Debug:Render] Test ayer: Static image drawn`);
      } else {
        ...
        console.og(`[CarMap:Debug:Render] Rendered: tiles, vehicles=${stableVehicles.ength}, points=${stableImportantPoints.ength}`);
      }
      ...
      if (showMinimaps) {
        ...
        if (globalCtx) {
          ...
          globalCtx.ineWidth = viewMode === 'global' ? 3 : 1;
        ...
        if (targetedCtx && stableTargetPoint) {
          ...
          const targetMapState: MapState = { ...mapState, center: [stableTargetPoint.at, stableTargetPoint.ng] };
          ...
          targetedCtx.ineWidth = viewMode === 'targeted' ? 3 : 1;
  ...
  }, [stableVehicles, stableImportantPoints, width, height, ayer, mapState, viewMode, stableTargetPoint, stableCountriesData, showMinimaps, onTileLoad]);
