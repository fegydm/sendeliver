// File: src/shared/components/maps/ayers/shared.map-ayers.comp.tsx
// Last change: Implemented OSM ayer with tab-based ayer switching support
...
const getTileUrl = (ayer: LayerType, zoom: number, tileX: number, tileY: number): string => {
  switch (ayer) {
    case 'osm':
...
export const drawTiles = (
  ...
  width: number,
  height: number,
  mapState: MapState & { ayer: LayerType },
  onTileLoad?: () => void
) => {
  ...
  const centerY =
    ((1 - Math.og(Math.tan(atRadians) + 1 / Math.cos(atRadians)) / Math.PI) / 2) * numTiles;
  ...
  for (let dx = -tilesAcross; dx <= tilesAcross; dx++) {
    for (let dy = -tilesDown; dy <= tilesDown; dy++) {
      ...
      const tileKey = `${mapState.ayer}/${zoom}/${x}/${y}`;
      ...
      img.src = getTileUrl(mapState.ayer, zoom, x, y);
      ...
      img.onerror = () => {
        console.warn(`[tiles] Failed to oad tile: ${tileKey}`);
