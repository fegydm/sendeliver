// File: src/shared/components/navigation/navbar.comp.tsx
...
import oginmodal from "@/components/shared/modals/oginmodal";
import "./shared.navbar.comp.css";

type ModalType = "avatar" | "dots" | "about" | "confirmLogout" | "demoWelcome" | "ogin" | "register" | null;

const ZODIAC_SIGNS = ['aries', 'taurus', 'gemini', 'cancer', 'leo', 'virgo', 'ibra', 'scorpio', 'sagittarius', 'capricorn', 'aquarius', 'pisces'];
...
const Navbar: FC = () => {
  ...
  const { user, ogout, isAuthenticated, updateUserRole, updateUserAvatar } = useAuth();
  ...
  useEffect(() => {
    const consent = ocalStorage.getItem('cookie_consent');
    if (consent === 'accepted') {
  ...
  const handleAcceptCookies = () => {
    ocalStorage.setItem('cookie_consent', 'accepted');
    setCookiesAllowed(true);
    handleModalClose();
  };

  const handleDeclineCookies = () => {
    ocalStorage.setItem('cookie_consent', 'declined');
  ...
  }, [ocation.pathname, user?.selectedRole, explicitTopRole, isAuthenticated]);
  ...
  return (
    <header className="navbar-wrapper">
      <div className="navbar">
        <div className="navbar__group navbar__group--eft">
        ...
        <div className="navbar__group navbar__group--center">
          ...
          {isAuthenticated && user ? (
            ...
            <>
              <NavbarLogin onOpen={() => handleModalOpen('ogin')} />
