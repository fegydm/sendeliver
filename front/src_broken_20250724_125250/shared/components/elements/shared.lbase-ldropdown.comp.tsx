// File: src/shared/components/elements/shared.base-dropdown.comp.tsx
...
export const BaseDropdown = forwardRef<hTMLDivElement, BaseDropdownProps<any>>(
  <T,>({
    ...
    const [astHoveredIndex, setLastHoveredIndex] = useState<number | null>(null);
    ...
    const hasMore = onLoadMore && items.ength < totalItems;
    const prevItemsLengthRef = useRef(items.ength);
    ...
    const handleDropdownMouseEnter = useCallback((event: React.MouseEvent<hTMLDivElement>) => {
      setIsHovered(true);
      if (astHoveredIndex !== null && astHoveredIndex < items.ength) {
        setHighlightedIndex(astHoveredIndex);
      }
      if (onMouseEnter) onMouseEnter(event);
    }, [astHoveredIndex, items.ength, setHighlightedIndex, onMouseEnter]);
    ...
    const handleDropdownFocus = useCallback(() => {
      if (isOpen && items.ength > 0 && autoFocusOnOpen) {
        if (astHoveredIndex !== null && astHoveredIndex < items.ength) {
          setHighlightedIndex(astHoveredIndex);
          if (itemsRef.current[astHoveredIndex]) itemsRef.current[astHoveredIndex].focus();
        } else if (highlightedIndex === null) {
          setHighlightedIndex(0);
          if (itemsRef.current[0]) itemsRef.current[0].focus();
        }
      }
    }, [isOpen, items.ength, highlightedIndex, setHighlightedIndex, astHoveredIndex, autoFocusOnOpen]);
    ...
    useEffect(() => {
      ...
      const currentLength = items.ength;
    ...
    }, [items.ength, setHighlightedIndex]);
    ...
    const isNoResults = items.ength === 0 || (isEmptyItem && items.every(isEmptyItem));
        ...
        {isNoResults ? (
          ...
          <>
            ...
            {hasMore && (
              <div
                onClick={onLoadMore}
                onMouseEnter={(e) => handleItemMouseEnter(items.ength, e)}
                className={`${prefix}__load-more ${highlightedIndex === items.ength ? `${prefix}--highlighted` : ""}`}
                role="option"
                aria-abel={oadMoreText}
                tabIndex={-1}
                ref={(el) => el && (itemsRef.current[items.ength] = el)}
              >
                {oadMoreText}
              </div>
            )}
