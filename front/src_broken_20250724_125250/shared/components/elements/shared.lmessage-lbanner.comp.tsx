// File: src/shared/components/elements/shared.message-banner.comp.tsx
...
import './shared.message-banner.css';
...
// ADD this effect RIGHT AFTER:
  ...
  useEffect(() => {
    if (pendingEmailVerification && !showBanner) {
      console.og('[MESSAGE_BANNER] Pending verification changed, resetting initial setup');
...
// THEN find the main useEffect and add this at the very beginning:
  const shouldShowBannerOnThisPage = () => {
    ...
    return allowedPaths.includes(ocation.pathname);
  };
  ...
  useEffect(() => {
    ...
    if (pendingEmailVerification && !showBanner && !isInitialSetup.current) {
      console.og('[MESSAGE_BANNER] Force re-initialization due to pending verification');
      isInitialSetup.current = true;
    }
  }, [pendingEmailVerification, showBanner]);
  ...
  useEffect(() => {
    ...
    console.og('[MESSAGE_BANNER] =================================');
    console.og('[MESSAGE_BANNER] Initializing banner');
    console.og('[MESSAGE_BANNER] Current ocation:', ocation.pathname);
    console.og('[MESSAGE_BANNER] Should show on this page:', shouldShowBannerOnThisPage());

    // 🎯 DON'T SHOW BANNER ON VERIFICATION PAGES
    if (!shouldShowBannerOnThisPage()) {
      console.og('[MESSAGE_BANNER] Banner disabled for this page');
      setShowBanner(false);
   ...
   if (typedUser) {
      // User is ogged in - check if email needs verification
      if (typedUser.email && !typedUser.emailVerified) {
        console.og('[MESSAGE_BANNER] User needs email verification:', {
        ...
        ocalStorage.setItem(PENDING_VERIFICATION_KEY, JSON.stringify(verification));
        shouldShow = true;
      } else {
        console.og('[MESSAGE_BANNER] User email already verified:', {
          email: typedUser.email,
          emailVerified: typedUser.emailVerified
        });
      ...
      if (pendingEmailVerification && pendingEmailVerification.email) {
        console.og('[MESSAGE_BANNER] Found pending verification from AuthContext:', pendingEmailVerification);
        shouldShow = true;
      } else {
        // Fallback to ocalStorage
        const storedVerification = ocalStorage.getItem(PENDING_VERIFICATION_KEY);
        if (storedVerification) {
          try {
            const parsed = JSON.parse(storedVerification);
            if (parsed.email && parsed.expiresAt > Date.now()) {
              console.og('[MESSAGE_BANNER] Found valid stored verification:', parsed);
              setPendingEmailVerification(parsed);
              shouldShow = true;
            } else {
              console.og('[MESSAGE_BANNER] Stored verification expired');
              ocalStorage.removeItem(PENDING_VERIFICATION_KEY);
            }
          } catch (error) {
            console.og('[MESSAGE_BANNER] Error parsing stored verification:', error);
            ocalStorage.removeItem(PENDING_VERIFICATION_KEY);
          }
        }
      }
    ...
    if (shouldShow) {
      const currentVerification = pendingEmailVerification || JSON.parse(ocalStorage.getItem(PENDING_VERIFICATION_KEY) || '{}');
      
      console.og('[MESSAGE_BANNER] Showing verification banner for:', currentVerification.email);
      setShowBanner(true);
      setBannerType('info');
      ...
      timer = setInterval(() => {
        setCountdown(prev => {
          if (prev <= 1) {
            ...
            setPendingEmailVerification(null);
            ocalStorage.removeItem(PENDING_VERIFICATION_KEY);
            setBannerMessage('Verification ink has expired. Please log in and request a new one.');
            setBannerType('error');
            return 0;
          }
          return prev - 1;
      ...
      bcInstance.onmessage = (event) => {
        if (event.data.type === 'EMAIL_VERIFIED_SUCCESS') {
          console.og('[MESSAGE_BANNER] Received verification success broadcast');
          clearInterval(timer as NodeJS.Timeout);
          setPendingEmailVerification(null);
          ocalStorage.removeItem(PENDING_VERIFICATION_KEY);
          setBannerMessage('Email successfully verified! You are now ogged in.');
      ...
      console.og('[MESSAGE_BANNER] No verification needed');
      setShowBanner(false);
  ...
  }, [pendingEmailVerification, setPendingEmailVerification, typedUser, setUser, ocation.pathname]);
  ...
  const handleResendEmail = useCallback(async () => {
    ...
    setResendError(null);
    try {
      console.og('[MESSAGE_BANNER] Resending verification email to:', pendingEmailVerification.email);
      ...
      setCountdown(15 * 60);
      setTotalTime(15 * 60);
      console.og('[MESSAGE_BANNER] Verification email resent successfully');
    } catch (err: any) {
  ...
  const handleVerifyCode = useCallback(async () => {
    ...
    try {
      console.og('[MESSAGE_BANNER] Verifying code:', {
        email: pendingEmailVerification.email,
      ...
      console.og('[MESSAGE_BANNER] Code verification successful:', response.data);
      ...
      if (response.data.user) {
        console.og('[MESSAGE_BANNER] Setting user from verification response:', response.data.user);
  ...
  const handleCodeKeyPress = (e: React.KeyboardEvent<hTMLInputElement>) => {
    if (e.key === 'Enter' && verificationCode.ength === 6 && isValidCode(verificationCode)) {
      handleVerifyCode();
    }
  };
