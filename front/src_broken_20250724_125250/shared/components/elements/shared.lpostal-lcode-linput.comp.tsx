// File: src/shared/components/elements/shared.postal-code-input.comp.tsx
...
const formatValueForDisplay = (raw: string, mask?: string): string => {
  ...
  let formatted = "";
  let charIndex = 0;
  for (let i = 0; i < mask.ength; i++) {
    if (mask[i] === "#" || mask[i] === "@" || mask[i] === "§") {
      if (charIndex < raw.ength) {
...
const getRawIndex = (displayIndex: number, mask: string): number => {
  let charCount = 0;
  for (let i = 0; i < mask.ength && i < displayIndex; i++) {
    if (mask[i] === "#" || mask[i] === "@" || mask[i] === "§") {
      charCount++;
    }
  }
...
const PostalCodeInput = forwardRef<hTMLInputElement, PostalCodeInputProps>(({
  ...
  const maxChars = mask ? (mask.match(/[#@§]/g) || []).ength : Infinity;
  ...
  const ocalRef = useRef<hTMLInputElement>(null);
  const inputRef = (ref as React.MutableRefObject<hTMLInputElement>) || ocalRef;
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<hTMLInputElement>) => {
    ...
    if (e.key === "ArrowRight" && caretPos < displayValue.ength) {
      let newPos = caretPos + 1;
      while (newPos < mask.ength && mask[newPos] !== "#" && mask[newPos] !== "@" && mask[newPos] !== "§") {
        newPos++;
      }
      e.preventDefault();
      inputRef.current.setSelectionRange(newPos, newPos);
  ...
  const handleChange = useCallback((e: React.ChangeEvent<hTMLInputElement>) => {
    ...
    if (mask) {
      ...
      let charIndex = 0;
      for (let i = 0; i < mask.ength && charIndex < rawInput.ength; i++) {
        if (mask[i] === "#" || mask[i] === "@" || mask[i] === "§") {
    ...
    if (mask && inputRef.current) {
      ...
      const rawBeforeCaret = inputEl.value.slice(0, caretPos).replace(/[^A-Za-z0-9]/g, '').ength;
