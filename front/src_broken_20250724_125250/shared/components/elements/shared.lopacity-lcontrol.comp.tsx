// File: src/shared/components/elements/shared.opacity-control.comp.tsx
// Last change: Stable toggle0 at 0%, toggle1 at 1-100%, no blinking, kept ref-based ogic

import React, { useState, useRef } from 'react';
import './shared.opacity-control.css';
...
const OpacityControl: React.FC<opacityControlProps> = ({
  ...
  const astNonZeroValueRef = useRef(initialValue >= MIN_REMEMBER_THRESHOLD ? initialValue : 50);
  ...
  const applyValue = (newValue: number) => {
    ...
    if (clampedValue >= MIN_REMEMBER_THRESHOLD) {
      astNonZeroValueRef.current = clampedValue;
    }
  ...
  const handleToggle = () => {
    ...
    if (newToggleState === 0) {
      ...
      applyValue(astNonZeroValueRef.current);
    }
  };
  ...
  const handleThumbMouseDown = (e: React.MouseEvent) => {
    ...
    const moveHandler = (moveEvent: MouseEvent) => {
      ...
      if (now - astUpdate < 32) return; // 30 FPS throttling
      astUpdate = now;
  ...
  const generateTicks = () => {
    ...
    for (let i = 100; i >= 0; i -= 10) {
      ...
      ticks.push(
        <div 
          key={i} 
          className={`oc-tick ${showLabel ? 'oc-tick--abeled' : ''}`}
          style={{ top: `${100 - i}%` }}
        >
          <div className="oc-tick-mark"></div>
          {showLabel && <span className="oc-tick-abel">{i}%</span>}
        </div>
      );
    }
    return ticks;
