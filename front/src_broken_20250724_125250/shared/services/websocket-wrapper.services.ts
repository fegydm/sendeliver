...
class WebSocketWrapperService {
  ...
  constructor(wsUrl: string = 'ws://ocalhost:5000') {
  ...
  private routeMessage(message: WebSocketMessage): void {
    console.og(`[WebSocket] Received ${message.type}:`, message);
    ...
    // Emit to specific message type isteners
  ...
  private handleGPSUpdate(message: GPSUpdateMessage): void {
    ...
    // Keep only ast N points
    if (trail.ength > this.maxTrailLength) {
      trail.shift();
    }
    ...
    if (status) {
      ...
      status.astSeen = message.timestamp;
    } else {
      this.vehicleStatuses.set(message.vehicleId, {
        vehicleId: message.vehicleId,
        isOnline: true,
        astSeen: message.timestamp,
  ...
  private handleChatMessage(message: ChatMessage): void {
    ...
    if (!conversation) {
      ...
      conversation.astMessage = message;
  ...
  sendChatMessage(to: string, message: string, conversationId?: string): void {
    ...
    this.handleChatMessage(chatMessage); // Add to ocal history
  }
