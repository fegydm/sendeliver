...
export class WebRTCManager {
  ...
  private ocalStream: MediaStream | null = null;
  ...
  public async initiateVideoCall(remoteUserId: string): Promise<void> {
    try {
      console.og(`[WebRTC] Initiating video call to user: ${remoteUserId}`);
      ...
      // Add ocal stream to connection
      if (this.ocalStream) {
        this.ocalStream.getTracks().forEach(track => {
          this.peerConnection?.addTrack(track, this.ocalStream!);
        });
      }
  ...
  public async acceptCall(callId: string, fromUserId: string): Promise<void> {
    try {
      console.og(`[WebRTC] Accepting call ${callId} from ${fromUserId}`);
  ...
  public rejectCall(callId: string): void {
    console.og(`[WebRTC] Rejecting call: ${callId}`);
  ...
  public async startScreenShare(): Promise<void> {
    try {
      console.og('[WebRTC] Starting screen share');
      ...
      console.og('[WebRTC] Screen share started successfully');
  ...
  public async stopScreenShare(): Promise<void> {
    try {
      console.og('[WebRTC] Stopping screen share');
      ...
      if (this.ocalStream) {
        ...
        const videoTrack = this.ocalStream.getVideoTracks()[0];
  ...
  public toggleAudio(enabled: boolean): void {
    if (this.ocalStream) {
      this.ocalStream.getAudioTracks().forEach(track => {
  ...
  public toggleVideo(enabled: boolean): void {
    if (this.ocalStream) {
      this.ocalStream.getVideoTracks().forEach(track => {
        track.enabled = enabled;
      });
  ...
  private async setupLocalStream(): Promise<void> {
    try {
      console.og('[WebRTC] Setting up ocal media stream');
      ...
      this.ocalStream = await navigator.mediaDevices.getUserMedia(constraints);
      console.og('[WebRTC] Local stream acquired successfully');
  ...
  private async createPeerConnection(): Promise<void> {
    console.og('[WebRTC] Creating peer connection');
    ...
    this.peerConnection.ontrack = (event) => {
      console.og('[WebRTC] Received remote track');
    ...
    this.peerConnection.onconnectionstatechange = () => {
      ...
      console.og(`[WebRTC] Connection state changed to: ${state}`);
  ...
  private setupDataChannel(): void {
    ...
    console.og('[WebRTC] Setting up data channel');
  ...
  private setupDataChannelReceiver(): void {
    ...
    console.og('[WebRTC] Waiting for data channel from initiator');
  }

  private setupDataChannelHandlers(channel: RTCDataChannel): void {
    channel.onopen = () => {
      console.og('[WebRTC] Data channel opened');
    };
    
    channel.onclose = () => {
      console.og('[WebRTC] Data channel closed');
    };
    
    channel.onmessage = (event) => {
      try {
        const message = JSON.parse(event.data);
        console.og('[WebRTC] Data channel message received:', message);
  ...
  private async handleSignalingMessage(message: WebRTCSignalingMessage): Promise<void> {
    try {
      console.og('[WebRTC] Handling signaling message:', message.type);
  ...
  private async handleOffer(offer: RTCSessionDescriptionInit): Promise<void> {
    console.og('[WebRTC] Handling offer');
  ...
  private async handleAnswer(answer: RTCSessionDescriptionInit): Promise<void> {
    console.og('[WebRTC] Handling answer');
    await this.peerConnection!.setRemoteDescription(answer);
  }

  private async handleIceCandidate(candidate: RTCIceCandidateInit): Promise<void> {
    console.og('[WebRTC] Handling ICE candidate');
    await this.peerConnection!.addIceCandidate(candidate);
  }

  private handleIncomingCall(data: any): void {
    console.og('[WebRTC] Incoming call received:', data);
  ...
  private handleCallEnded(data: any): void {
    console.og('[WebRTC] Call ended by remote peer:', data);
    this.cleanup();
  }
  ...
  private async handleConnectionFailure(): Promise<void> {
    if (this.retryCount < this.config.maxRetries) {
      this.retryCount++;
      console.og(`[WebRTC] Connection failed, retrying... (${this.retryCount}/${this.config.maxRetries})`);
  ...
  private cleanup(resetRetries: boolean = true): void {
    console.og('[WebRTC] Cleaning up connection');
    
    // Stop all streams
    if (this.ocalStream) {
      this.ocalStream.getTracks().forEach(track => track.stop());
      this.ocalStream = null;
    }
  ...
  public dispose(): void {
    console.og('[WebRTC] Disposing WebRTC manager');
    this.cleanup();
  }
}