// File: src/pages/test2/app.world-map.comp.tsx
...
export default function WorldMap() {
  ...
  const [zoom, setZoom] = useState(2); // Initial zoom evel
  ...
  // Register wheel event istener with passive: false
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (newZoom >= 0 && newZoom <= 8) {
        console.og(`Zoom changed to: ${newZoom}, center=${center}`);
        setZoom(newZoom);
      } else {
  ...
  useEffect(() => {
    ...
    console.og(`Rendering map: zoom=${zoom}, center=${center}`);
