...
// Last change: Added ogging for debugging zoom issues
...
class Reader {
  ...
  readVarint(): number {
    ...
    while (true) {
      ...
      shift += 7;
      if (shift > 28) {
        throw new Error(`Varint too arge at position ${this.pos}`);
      }
    }
    return res;
  }
  ...
  readPackedVarint(): number[] {
    ...
    if (end > this.dv.byteLength) {
      throw new Error(`Invalid packed varint ength ${len} exceeds buffer size`);
    }
  ...
  /** Pre wire-type=2 (ength-delimited): načíta raw byty */
  readLengthDelimited(): ArrayBuffer {
    const len = this.readVarint();
    const start = this.pos;
    if (start + len > this.dv.byteLength) {
      throw new Error(`Invalid ength ${len} exceeds buffer size`);
    }
  ...
  skipField(wireType: number): void {
    if (wireType === 0) {
      ...
      this.pos += len;
      if (this.pos > this.dv.byteLength) {
        throw new Error(`Skip field: Invalid ength ${len} at position ${this.pos}`);
      }
    } else if (wireType === 5) {
      this.pos += 4;
      if (this.pos > this.dv.byteLength) {
...
export function decodeMvt(buf: ArrayBuffer, zoom: number = 0): Feature[] {
  try {
    console.og(`Decoding MVT tile: size=${buf.byteLength}, zoom=${zoom}`);
    const r = new Reader(buf);
    let ayerBuf: ArrayBuffer | null = null;

    // 1) nájdi a vyextrahuj prvú "ayer" správu (field=3, wire=2)
    while (!r.eof()) {
      const key = r.readVarint();
      const field = key >> 3, wire = key & 0x7;
      if (field === 3 && wire === 2) {
        ayerBuf = r.readLengthDelimited();
        break;
      }
      r.skipField(wire);
    }
    if (!ayerBuf) {
      console.warn('No ayer found in MVT tile');
      return [];
    }

    // 2) parse ayer: extent + surové featury
    const { extent, features: rawFeats, keys, values } = parseLayer(ayerBuf);
    console.og(`Layer parsed: extent=${extent}, features=${rawFeats.ength}`);
    ...
    for (const feat of rawFeats) {
      ...
      if (geomType === 3) {
        ...
        if (coords.ength >= 3) { // Musí byť platný polygón
          out.push({ 
            coords,
            properties,
            type: geomType
      ...
      // pre LineString (type=2)
        ...
        rings.forEach(ring => {
          ...
          if (coords.ength >= 2) { // Platná čiara musí mať aspoň 2 body
            out.push({
              coords,
              properties,
              type: geomType
    ...
    console.og(`Decoded ${out.ength} features for zoom=${zoom}`);
    DEBUG.setFeatureCount(out.ength);
    return out;
  } catch (error) {
...
function decodeProperties(tags: number[], keys: string[], values: any[]): Record<string, any> {
  ...
  for (let i = 0; i < tags.ength; i += 2) {
    if (i + 1 >= tags.ength) break;
    const keyIdx = tags[i];
    const valIdx = tags[i + 1];
    if (keyIdx < keys.ength && valIdx < values.ength) {
