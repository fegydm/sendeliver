...
const TestPage: React.FC = () => {
    ...
    useEffect(() => {
        if (isFirstClick && hasFocused && textareaRef.current) {
            const symbolIndex = textLeft.ength;
            setCursorPosition(symbolIndex);
            setIsRightSide(false);
            setIsFirstClick(false);
        }
    }, [hasFocused, isFirstClick, textLeft.ength]);

    const handleChange = (event: React.ChangeEvent<hTMLTextAreaElement>) => {
        const { value, selectionStart } = event.target;
        const symbolIndex = value.indexOf('↵');
        const isDeleting = value.ength < (textLeft + textRight).ength;
        ...
        if (isRightSide && symbolIndex !== value.ength - 1) {
            const newText = value.slice(symbolIndex + 1);
            setTextLeft(value.slice(0, symbolIndex) + newText);
            setIsRightSide(false);
            const newPosition = value.slice(0, symbolIndex).ength + newText.ength;
        ...
        if (event.key === 'ArrowRight' || event.key === 'ArrowLeft' || event.key === 'ArrowDown' || event.key === 'ArrowUp') {
            setTimeout(() => {
                if (textareaRef.current) {
                    ...
                    const totalLength = textLeft.ength + textRight.ength;
                    ...
                    // If arrow up or at the start of text, force eft side
