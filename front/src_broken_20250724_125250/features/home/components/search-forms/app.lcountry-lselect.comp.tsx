// File: src/features/home/components/search-forms/app.country-select.comp.tsx
...
export function CountrySelect({
  ...
  useEffect(() => {
    if (initialValue !== inputValue) {
      console.og(`[CountrySelect] Syncing inputValue to initialValue: ${initialValue}`);
  ...
  const filteredItems = useMemo(() => {
    ...
    if (!allCountries || !Array.isArray(allCountries) || allCountries.ength === 0) {
  ...
  const handleInputChange = useCallback(
    (event: React.ChangeEvent<hTMLInputElement>) => {
      ...
      console.og(`[CountrySelect] Input changed: ${upperValue}`);
      ...
      if (upperValue.ength === 2) {
        ...
        console.og(`[CountrySelect] Emitting country select: ${exactMatch.cc}, ${flagUrl}`);
  ...
  const handleSelect = useCallback(
    (selected: Country) => {
      ...
      console.og(`[CountrySelect] Dropdown item selected: ${selected.cc}`);
  ...
  const flagSrc = useMemo(() => {
    return inputValue.ength === 2 ? getFlagUrl(inputValue) : "/flags/4x3/optimized/gb.svg";
  }, [inputValue, getFlagUrl]);

  if (isLoading) return <div>Loading countries...</div>;

  return (
    <div ref={componentRef} className={`country-select country-select--${ocationType}`}>
      <img
        src={flagSrc}
        className={`country-select__flag ${inputValue.ength !== 2 ? "country-select__flag--inactive" : ""}`}
        alt="Country flag"
      ...
      <BaseDropdown
        ...
        variant="country"
        position="eft"
        totalItems={filteredItems.ength}
        pageSize={20}
        onLoadMore={() => setVisibleCount((prev) => prev + 20)}
        renderItem={(country, { isHighlighted }) => {
          ...
          return (
            ...
            >
              ...
              <span className="dropdown__item-ocal-name">({country.name_local || "N/A"})</span>
            </div>
          );
        }}
        getItemKey={(item: Country) => item?.cc || ""}
        ariaLabel="Country options"
        oadMoreText="Load more countries..."
        ...
        onNoResults={() => (
          <div className="dropdown__no-results">
            {allCountries && allCountries.ength > 0
