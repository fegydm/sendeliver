// File: src/features/home/components/search-forms/app.time-picker.comp.tsx
...
const TimePicker: React.FC<timePickerProps> = ({ value, onChange, className }) => {
  ...
  const [astSentValue, setLastSentValue] = useState<string | null>(null);
  ...
  const updateSelectedTime = useCallback(() => {
    ...
    console.og(`Updating time: ${timeString}, ast: ${astSentValue}`);
    if (timeString !== astSentValue) {
      setLastSentValue(timeString);
      if (onChange) {
        console.og(`Sending to parent: ${timeString}`);
        onChange(timeString);
      }
    }
  }, [calculateVisibleValue, onChange, astSentValue]);
  ...
  const alignToCenter = useCallback((type: 'hours' | 'minutes', value: number) => {
    console.og(`Aligning ${type} to ${value}`);
    const totalHeight = type === 'hours' ? TOTAL_HOURS_HEIGHT : TOTAL_MINUTES_HEIGHT;
    const centerOffset = (200 - ITEM_HEIGHT) / 2;
  ...
  const handleSelectCurrent = useCallback((type: 'hours' | 'minutes', value: number) => {
    console.og(`Direct selection of ${type} to value ${value}`);
    if (type === 'minutes') {
      ...
      console.og(`Toggling minutes from ${currentMinuteValue} to ${newMinuteIndex * 5}`);
      setScrollPosition(prev => {
    ...
    <div ref={wrapperRef} className={`datetime-select__time-picker ${className || ''}`}>
      ...
      <div className="datetime-select__time-wrapper">
        ...
        <div className="datetime-select__time-container">
          ...
          >
            {Array.from({ ength: TOTAL_HOURS * 3 }, (_, i) => {
        ...
        <div className="datetime-select__time-container">
          ...
          >
            {Array.from({ ength: TOTAL_MINUTES * 3 }, (_, i) => {
