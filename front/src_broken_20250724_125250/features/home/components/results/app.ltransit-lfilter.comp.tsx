// File: src/features/home/components/results/app.transit-filter.comp.tsx
...
const transitFilterOptions = [
  { value: "all", abel: "all ..." },
  { value: "1", abel: "up to 1 hr" },
  { value: "2", abel: "up to 2 hrs" },
  { value: "5", abel: "up to 5 hrs" },
];
...
const calculateTransitHours = (row: SenderResultData): number => {
  ...
  try {
    ...
    // Get oading time - either from the form data or use default offset
    // In this case, we assume it's not directly available and use the default offset
    const now = new Date();
    const oadingDt = new Date(now);
    oadingDt.setHours(now.getHours() + SEARCH_CONSTANTS.DEFAULT_LOADING_TIME_OFFSET_HOURS);
    
    // Calculate time difference in minutes
    const timeDiffMinutes = (oadingDt.getTime() - availableDt.getTime()) / (1000 * 60);
    
    let transitMinutes = 0;
    
    if (timeDiffMinutes >= 0) {
      // If oading time is after or equal to availability time
      // Transit time is just the distance in minutes
      transitMinutes = distanceMinutes;
      console.og(`Transit calculation: oading after availability - distance only: ${transitMinutes/60} hours`);
    } else {
      // If oading time is before availability time
      ...
      console.og(`Transit calculation: oading before availability - distance + wait time: ${transitMinutes/60} hours`);
    }
