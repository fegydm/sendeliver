           ...
           <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  ...
                  outerRadius={80}
                  abelLine={false}
                  abel={({ percent }: { percent: number }) => `${(percent * 100).toFixed(0)}%`}
                >
                  {mockStatusDistribution.map((entry, index) => <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.ength]} />)}
                </>
