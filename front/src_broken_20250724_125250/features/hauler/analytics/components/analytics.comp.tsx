// File: src/features/hauler/hauler.analytics.comps/analytics.comp.tsx
...
import './hauler.analytics.comp.css';
          ...
          <div className="chart-grid chart-grid--three-columns">
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={mockRevenueByType} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} abel>
                    {mockRevenueByType.map((entry, index) => <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.ength]} />)}
                  </>
                  <Tooltip formatter={(value: number) => formatNumber(value, '€')} />
                </>
              </>
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <PieChart>
                  <Pie data={mockCostBreakdown} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={80} abel>
                    {mockCostBreakdown.map((entry, index) => <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.ength]} />)}
                  </>
                  <Tooltip formatter={(value: number) => formatNumber(value, '€')} />
                </>
              </>
