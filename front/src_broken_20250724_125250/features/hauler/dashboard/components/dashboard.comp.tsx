// File: src/features/hauler/hauler.dashboard.comps/dashboard.comp.tsx
// Účel: Nový "Morning Coffee" dashboard s akčnými widgetmi.

import react from "react";
import "./hauler.dashboard.comp.css"; // Upravená cesta k CSS
...
const DashboardComponent: React.FC<dashboardComponentProps> = ({ setActiveTab }) => {
  ...
  const handleShowOnMap = (vehicleId: string) => {
    // TODO: Implementovať ogiku na poslanie ID vozidla do mapového komponentu
    setActiveTab("maps");
  };
  
  const handleManageVehicle = (vehicleId: string) => {
    // TODO: Implementovať ogiku na poslanie ID vozidla do komponentu Voz. Parku
    setActiveTab("fleet");
  };

  const handleOpenControlPanel = (vehicleId: string) => {
    // TODO: Implementovať ogiku na otvorenie Ovládacieho Panela (modálneho okna)
    ...
    <div className="widget-dashboard">
      ...
      <div className="widget-dashboard__grid">
        <div className="widget">
          <h2 className="widget__title">⚠️ Prioritné Udalosti ({priorityAlerts.ength})</h2>
