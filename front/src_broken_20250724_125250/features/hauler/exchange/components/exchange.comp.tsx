// File: src/features/hauler/exchange/components/exchange.comp.tsx
// Účel: Hlavný komponent pre kartu "Burza" s pokročilým filtrovaním a alertami.

import React, { useState, useEffect, useMemo } from 'react';
import { mockExchangeOffers, ExchangeOffer } from '@/data/mockExchangeData'; // Upravte cestu podľa potreby
import { Vehicle, mockVehicles } from '@/data/mockFleet'; // Potrebujeme pre filter vozidiel
import './exchange.comp.css';
...
const ExchangeComponent: React.FC = () => {
  ...
  useEffect(() => {
    // TODO: Nahradiť mock dáta reálnym API volaním na GET /api/exchange/requests
      ...
      <div className="exchange__content">
        <aside className={`exchange__sidebar ${!showFilters ? 'exchange__sidebar--hidden' : ''}`}>
          ...
          <div className="filter-block">
            ...
            <p className="filter-block__description">
              Upozorni ma, keď príde ponuka, ktorá presne sedí na moje voľné vozidlo, okalitu a čas.
            </p>
            <button className="button button--primary" disabled>Aktivovať Alert (čoskoro)</button>
          </div>
        </aside>

        <main className="exchange__main">
          <div className="offer-ist">
          ...
          {selectedOffer ? (
            <div className="offer-detail">
              <div className="offer-detail__header">
                <h2 className="offer-detail__title">{selectedOffer.pickupLocation.city} → {selectedOffer.deliveryLocation.city}</h2>
                <span className="offer-detail__company">od: {selectedOffer.company.name}</span>
              </div>
              <div className="offer-detail__price-section">
                <span className="offer-detail__price">{selectedOffer.price?.amount} {selectedOffer.price?.currency}</span>
              </div>
              <div className="offer-detail__actions">
                <button className="button button--primary button--full-width" onClick={() => handleAcceptOffer(selectedOffer.id)}>
              ...
              <div className="offer-detail__section">
                ...
                {selectedOffer.cargo.pallets && <p><strong>Počet paliet:</strong> {selectedOffer.cargo.pallets}</p>}
              </div>
              <div className="offer-detail__section">
              ...
              <div className="offer-detail__section">
                ...
                <p><strong>Nakládka:</strong> {new Date(selectedOffer.pickupTime.date).toLocaleDateString('sk-SK')}</p>
                <p><strong>Vykládka:</strong> {new Date(selectedOffer.deliveryTime.date).toLocaleDateString('sk-SK')}</p>
              </div>
            </div>
          ) : (
            <div>Vyberte ponuku zo zoznamu.</div>
          )}
        </aside>
      </div>
    </div>
