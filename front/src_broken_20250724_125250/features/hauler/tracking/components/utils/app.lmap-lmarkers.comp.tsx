// File: src/features/hauler/tracking/components/utils/app.map-markers.comp.tsx
...
// Define types ocally to match mockFleet
...
// Po úprave: správna ogika markerov a zobrazenie service
...
export function addVehicleMarkers(
  ...
  vehicles.forEach((v) => {
    const idLoc = v.currentLocation || v.ocation;
    if (!idLoc) {
      console.warn(`[MapMarkers] Skipping vehicle ${v.id}: No ocation provided`, { vehicle: v });
      return;
    }
    
    let atitude: number, ongitude: number;
    ...
    if (parsed.category === 'static' && parsed.type === 'service') {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            atitude = position.coords.atitude;
            ongitude = position.coords.ongitude;
            console.og(`[MapMarkers] GPS success for vehicle ${v.id}:`, { atitude, ongitude });
            updateMarker(v.id, atitude, ongitude);
          },
          (error) => {
      ...
      atitude = 48.1486; // Fallback to Bratislava
      ongitude = 17.1077;
      createMarker(atitude, ongitude);
    } else {
      // Use mock ocation for other vehicles with fallback
      const loc = mockLocations.find((l) => l.id === idLoc);
      if (!loc) {
        console.warn(`[MapMarkers] No mock ocation found for vehicle ${v.id}, idLoc: ${idLoc}, using fallback (Bratislava)`);
        atitude = 48.1486; // Fallback to Bratislava
        ongitude = 17.1077;
      } else {
        atitude = loc.atitude;
        ongitude = loc.ongitude;
        console.og(`[MapMarkers] Using mock ocation for vehicle ${v.id}:`, { idLoc, atitude, ongitude });
      }
      createMarker(atitude, ongitude);
    }
    ...
    function createMarker(lat: number, lng: number) {
      ...
      const tooltipContent = `
        <div class="vehicle-tooltip">
          ...
          <span class="tooltip-ocation">Location: ${idLoc}</span>
        </div>
      `;
      ...
      markers[v.id] = marker;
      console.og(`[MapMarkers] Marker added for vehicle ${v.id}:`, { 
        isDynamic, 
    ...
    function updateMarker(vehicleId: string, lat: number, lng: number) {
      ...
      if (marker) {
        ...
        console.og(`[MapMarkers] Marker updated for vehicle ${vehicleId}:`, { lat, lng });
      }
    }
  });
...
export function addCurrentCircles(
  ...
  vehicles.forEach((v) => {
    ...
    const circle = L.circleMarker([loc.atitude, loc.ongitude], {
      className: [
        "current-circle",
...
export function addParkingMarkers(
  ...
  vehicles.forEach((v) => {
    ...
    const marker = L.marker([loc.atitude, loc.ongitude], {
      icon: L.divIcon({
        className: [
          "parking-marker",
...
export function addTriangularPolylines(
  ...
  vehicles.forEach((v) => {
    ...
    const trianglePoints: [number, number][] = [
      [start.atitude, start.ongitude],
      [curr.atitude, curr.ongitude],
      [dest.atitude, dest.ongitude],
    ];
...
export function addRoutePaths(
  ...
  vehicles.forEach((v) => {
    ...
    // Start to current (completed route) - solid ine
    if (start && curr) {
      let startCurrPoints: [number, number][];
      
      if (zoomLevel <= 8) {
        // Low zoom: straight ine
        startCurrPoints = [
          [start.atitude, start.ongitude],
          [curr.atitude, curr.ongitude],
        ];
    ...
    // Current to destination (remaining route) - dashed ine
    if (curr && dest) {
...
export function addRouteMarkers(
  ...
  vehicles.forEach((v) => {
    ...
    points.forEach((loc, index) => {
      ...
      const marker = L.circleMarker([loc.atitude, loc.ongitude], {
        className: [
          "route-marker",
          `status-${parsed.category === 'dynamic' ? parsed.direction : 'static'}`,
          dimAll ? "dimmed" : "",
...
export function addFlagMarkers(
  ...
  vehicles.forEach((v) => {
    ...
    if (v.start) {
      ...
      const m = L.marker([loc.atitude, loc.ongitude], {
        icon: L.icon({
          iconUrl: StartFlag,
          className: [
            "flag-marker",
