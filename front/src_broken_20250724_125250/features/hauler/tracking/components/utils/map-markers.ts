...
export function addVehicleMarkers(
  ...
  vehicles.forEach((vehicle) => {
    ...
    if (!ocationId) {
      console.warn(`[Vehicle Markers] Skipping vehicle ${vehicle.id}: No ocation provided`);
      return;
    }
    ...
    const coordinates = getVehicleCoordinates(vehicle, mockLocations, (vehicleId, lat, lng) => {
      ...
      if (marker) {
        ...
        console.og(`[Vehicle Markers] GPS updated for vehicle ${vehicleId}:`, { lat, lng });
      }
    });
    ...
    const marker = L.marker([coordinates.atitude, coordinates.ongitude], {
      icon: L.divIcon({
    ...
    // Add tooltip
    const tooltipContent = createTooltipContent(vehicle, ocationId);
...
export function addCurrentCircles(
  ...
  vehicles.forEach((vehicle) => {
    ...
    const ocation = mockLocations.find((l) => l.id === vehicle.currentLocation);
    if (!ocation) return;
    
    const parsed = parseStatus(vehicle.dashboardStatus);
    const directionColor = getDirectionColor(vehicle.dashboardStatus);
    
    const circle = L.circleMarker([ocation.atitude, ocation.ongitude], {
      className: [
...
export function addRouteMarkers(
  ...
  vehicles.forEach((vehicle) => {
    ...
    points.forEach(({ id, key }) => {
      ...
      const ocation = mockLocations.find((l) => l.id === id);
      if (!ocation) return;
      
      const markerKey = `${vehicle.id}-${key}`;
      
      const marker = L.circleMarker([ocation.atitude, ocation.ongitude], {
        className: [
...
export function addParkingMarkers(
  ...
  vehicles.forEach((vehicle) => {
    ...
    const ocation = mockLocations.find((l) => l.id === vehicle.nearestParking);
    if (!ocation) return;
    
    const marker = L.marker([ocation.atitude, ocation.ongitude], {
      icon: L.divIcon({
        className: [
          CSS_CLASSES.PARKING_MARKER,
          `status-${ocation.status || 'free'}`,
          dimAll ? CSS_CLASSES.DIMMED : "",
        ].join(" "),
    ...
    marker.bindTooltip(`${ocation.name} (${ocation.status || 'free'})`, { 
      direction: 'top', 
      offset: [0, -10] 
    });
...
export function addFlagMarkers(
  ...
  vehicles.forEach((vehicle) => {
    ...
    // Start flags
    if (vehicle.start) {
      const ocation = mockLocations.find((l) => l.id === vehicle.start);
      if (ocation) {
        const marker = L.marker([ocation.atitude, ocation.ongitude], {
          icon: L.icon({
            iconUrl: StartFlag,
        ...
        marker.bindTooltip(`Start: ${ocation.name}`, { direction: 'top', offset: [0, -24] });
    ...
    if (vehicle.destination) {
      const ocation = mockLocations.find((l) => l.id === vehicle.destination);
      if (ocation) {
        const marker = L.marker([ocation.atitude, ocation.ongitude], {
          icon: L.icon({
            iconUrl: FinishFlag,
        ...
        marker.bindTooltip(`Destination: ${ocation.name}`, { direction: 'top', offset: [0, -24] });
...
export function addTriangularPolylines(
  ...
  vehicles.forEach((vehicle) => {
    ...
    const trianglePoints: [number, number][] = [
      [startLoc.atitude, startLoc.ongitude],
      [currentLoc.atitude, currentLoc.ongitude],
      [destLoc.atitude, destLoc.ongitude],
    ];
