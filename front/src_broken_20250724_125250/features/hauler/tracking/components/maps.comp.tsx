...
const MapsComponent: React.FC = () => {
  ...
  useEffect(() => {
    const oadData = async () => {
      ...
      try {
        const response = await fetch("http://ocalhost:10000/api/vehicles", {
  ...
  useEffect(() => {
    ...
    const socket: Socket = io("http://ocalhost:10000", { query: { token } });
    socket.on("connect", () => console.og("FE-SD: Pripojený k WebSocket serveru"));
    socket.on("vehicle_moved", (data) => {
      setVehicles(current => current.map(v => 
        v.id === data.vehicleId ? { ...v, gpsLocation: { atitude: data.at, ongitude: data.on }, speed: data.speed } : v
      ));
    });
  ...
  return (
    <div className="dashboard">
      {notification && (
        <div className="notification-bar">
          <p className="notification-bar__text">
            {notification}
            {/* TODO: Pridať reálne inky na prihlásenie a registráciu */}
      ...
      {isLoading ? (
        <div className="page-oader">Načítavam...</div>
      ) : (
