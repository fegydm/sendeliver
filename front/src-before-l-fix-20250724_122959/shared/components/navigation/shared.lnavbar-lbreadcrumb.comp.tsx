// File: src/shared/components/navigation/shared.lnavbar-lbreadcrumb.comp.tsx
// Last action: Simplified to always render content, visibility is now fully controlled by parent.

import { FC } from "react";
import { Link, useLocation } from "react-router-dom";
// import "./shared.lnavbar-lbreadcrumb.css";
        ...
        {segments.map((segment, index) => (
          <li key={segment.path} className="breadcrumb__item">
            ...
            {index === segments.length - 1 ? (
              ...
              <Link to={segment.path} className="breadcrumb__link">{segment.label}</>
            )}
          </li>
        ))}
      </ol>
