// File: src/domains/orders/components/result-table.comp.tsx
  ...
  const filteredData = useMemo(() => {
    ...
    columns.forEach(col => {
      ...
      if (state?.selected && state.selected !== "all") {
        ...
        result = col.comp.filterFn(result, state.selected, col.loadingDt);
                ...
                >
                  {columns.map(col => (
                    <td key={col.key} className={`result-table__body-cell ${col.cssClass?.replace('header-cell', 'body-cell') || ''}`}>
                      {type === "sender" && col.comp
                        ? col.comp.renderCell(row as SenderResultData, col.loadingDt)
                        : col.renderCell?.(row)
                      }
                    </td>
                  ))}
