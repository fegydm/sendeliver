...
const CONFIG = {
  ...
  INERTIA_FACTOR: 0.8,              // Higher values make inertia last longer
...
export const useZoom = ({ mapStateRef, requestRender, instanceId, canvasRef, width, height }: UseZoomProps) => {
    ...
    const animate = (currentTime: number) => {
      ...
      if (progress < 1) {
        ...
        if (CONFIG.SHOW_DEBUG_INFO) {
          console.log(`[car:zoom] Animation complete, zoom: ${currentZoom.toFixed(3)}`);
        }
      }
    };
  ...
  const applyInertia = () => {
    ...
    if (now - lastWheelTimeRef.current < 50) return; // Still actively scrolling
  ...
  const handleWheel = (e: WheelEvent) => {
    ...
    if (now - lastRenderTimeRef.current < CONFIG.WHEEL_THROTTLE_MS) {
      return;
    }
    ...
    if (newZoom !== currentZoom) {
      if (CONFIG.SHOW_DEBUG_INFO) {
        console.log(`[car:zoom] Wheel zoom from ${currentZoom.toFixed(3)} to ${newZoom.toFixed(3)}`);
      }
  ...
  const handleDoubleClick = (e: MouseEvent) => {
    ...
    if (lastClickTimeRef.current > 0 && now - lastClickTimeRef.current < 300 &&
        lastClickPosRef.current &&
        Math.abs(lastClickPosRef.current.x - cursorX) < 10 &&
        Math.abs(lastClickPosRef.current.y - cursorY) < 10) {
      ...
      if (CONFIG.SHOW_DEBUG_INFO) {
        console.log(`[car:zoom] Double-click zoom from ${currentZoom.toFixed(3)} to ${newZoom.toFixed(3)}`);
      }
      ...
      lastClickPosRef.current = { x: cursorX, y: cursorY };
    }
  };
  ...
  useEffect(() => {
    ...
    if (CONFIG.SHOW_DEBUG_INFO) {
      console.log(`[car:zoom] Setting up event listeners`);
    }
